...
namespace SharpBoot
{
    ...
    public partial class GenIsoFrm : Form
    {
        ...
        private void OnDataReceived(object sender, DataReceivedEventArgs e)
        {
            ...
            {
                ricGeniso.Invoke(new MethodInvoker(() => ricGeniso.Show()));
            }
            ...
            {
                var dcpercent = Convert.ToDecimal(e.Data.Substring(0, 5));
                ChangeProgress((int)dcpercent, 100,
                    Strings.CopyISOfiles + "\t" + e.Data.Substring(0, 5) + "%");
              
            }

            else if (e.Data != null && !(e.Data.Contains("%")))
            {
                ErrorMessage = e.Data;
                if (ricGeniso.InvokeRequired)
                {

                    ricGeniso.Invoke(new MethodInvoker(() => ricGeniso.AppendText(ErrorMessage + "\r\n") ));
                    
                }

                else
                {
                    ricGeniso.AppendText(ErrorMessage + "\r\n");
